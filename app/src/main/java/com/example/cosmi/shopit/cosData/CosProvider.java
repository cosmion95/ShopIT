...
import com.example.cosmi.shopit.data.ItemProvider;
...
public class CosProvider extends ContentProvider {
    ...
    static {
        ...
        sUriMatcher.addURI(CosContract.CONTENT_AUTHORITY, CosContract.PATH_ITEMS, ITEMS);

        //acces single row in items table
        sUriMatcher.addURI(CosContract.CONTENT_AUTHORITY, CosContract.PATH_ITEMS + "/#", ITEM_ID);
    }
    ...
    private Uri insertItem(Uri uri, ContentValues values) {
        ...
        String name = values.getAsString(CosContract.CosEntry.COLUMN_NAME);
        if (name == null){
        ...
        Integer price = values.getAsInteger(CosContract.CosEntry.COLUMN_PRICE);
        ...
        long id = database.insert(CosContract.CosEntry.TABLE_NAME, null, values);
    ...
    private int updateItem(Uri uri, ContentValues contentValues, String selection, String[] selectionArgs) {
        ...
        if (contentValues.containsKey(CosContract.CosEntry.COLUMN_NAME)) {
            String name = contentValues.getAsString(CosContract.CosEntry.COLUMN_NAME);
            if (name == null) {
        ...
        if (contentValues.containsKey(CosContract.CosEntry.COLUMN_PRICE)) {
            Integer price = contentValues.getAsInteger(CosContract.CosEntry.COLUMN_PRICE);
            if (price == null) {
        ...
        SQLiteDatabase database = itemDBHelper.getWritableDatabase();
        int rowsUpdated = database.update(CosContract.CosEntry.TABLE_NAME, contentValues, selection, selectionArgs);
    ...
    public String getType(@NonNull Uri uri) {
        final int match = sUriMatcher.match(uri);
        switch (match) {
            case ITEMS: return CosContract.CosEntry.CONTENT_LIST_TYPE;
            case ITEM_ID: return CosContract.CosEntry.CONTENT_ITEM_TYPE;
